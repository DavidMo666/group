...
Coverage output is generated by Istanbul in `coverage/`, including `coverage/lcov.info` for Codecov and `coverage/lcov-report/index.html` for a local HTML report. The Jest configuration enforces an 88% global coverage threshold across branches, functions, lines, and statements.
